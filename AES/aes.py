...
def mix_columns(state):
    ...
    for col in range(4):
        ...
        state[0][col] = galois_mult(s0, 2) ^ galois_mult(s1, 3) ^ s2 ^ s3
        state[1][col] = s0 ^ galois_mult(s1, 2) ^ galois_mult(s2, 3) ^ s3
        state[2][col] = s0 ^ s1 ^ galois_mult(s2, 2) ^ galois_mult(s3, 3)
